...
static __poll_t local_poll(struct file *filep, struct poll_table_struct *wait)
{
    __poll_t mask = 0;
    struct local_dev_t *locdev = filep->private_data;
    /* 把等待队列加入poll table中（这样当等待队列被唤醒时，调用select/poll的进程也会被唤醒） */
    poll_wait(filep, &locdev->rwq, wait);
    if ((atomic_read(&locdev->key_stat) != KEY_KEEP))
...
static ssize_t hello_read(struct file *filep, char __user *buf, size_t size, loff_t *pos)
{
    ...
    statbuf = (char)stat;
    size = copy_to_user(buf, &statbuf, 1)?-EFAULT:1;
    printk("key val:%d\r\n", stat);
    return size;
}

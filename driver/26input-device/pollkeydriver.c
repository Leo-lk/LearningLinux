...
/*
本例程使用poll轮询后发送input事件，上报按键按下的状态
它是通用input的子类
...
struct local_dev_t {
    struct input_polled_dev *inputpolldev;
...
/* For 按键消抖 */
static void btn_poll(struct input_polled_dev *dev)
{
    ...
    /* 只在按键按下和释放时上报input事件 */
    input_report_key(locdev->inputpolldev->input, BTN_1, stat);
	input_sync(locdev->inputpolldev->input);
}
...
int inputkey_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "driver key probe %s\n", __func__);
    ...
    /* 分配设备，可能会陷入睡眠 */
    local_dev->inputpolldev = input_allocate_polled_device();
    if (!local_dev->inputpolldev) {
		ret = -ENOMEM;
		pr_err( "%s: Not enough memory.\n", __func__);
		goto free_gpio;
    }
    local_dev->inputpolldev->poll = btn_poll;
    local_dev->inputpolldev->poll_interval = 200;
    local_dev->inputpolldev->private = local_dev;
    
    inputdev = local_dev->inputpolldev->input;
    ...
    ret = input_register_polled_device(local_dev->inputpolldev);
    if (ret) {
		pr_err( "%s: Failed to register device\n", __func__);
		goto free_inputpolldev;
	}
...
free_inputpolldev:
    ...
    input_free_polled_device(local_dev->inputpolldev);
free_gpio:
...
int inputkey_remove(struct platform_device *pdev)
{
    ...
    input_unregister_polled_device(localdev->inputpolldev);
    return 0;
}

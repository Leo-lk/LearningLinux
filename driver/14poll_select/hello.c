...
static int flag = 0;
DEFINE_MUTEX(mtx_lock);


wait_queue_head_t rwq, wwq;

int user_readable(void)
{
    return flag==1;
}
int user_writable(void)
{
    return flag==0;
}
...
{
    printk("kk hello release, flag = %d\n", flag);
...
{
    int error;
    wait_event_interruptible(rwq, flag==1);
...
static ssize_t hello_write(struct file *filep, const char __user *buf, size_t size, loff_t *pos)
{
    ...
    wait_event_interruptible(wwq, flag==0);
...
__poll_t hello_poll (struct file * filep, struct poll_table_struct * poll_tab)
{
    u32 mask = 0;
    mutex_lock(&mtx_lock);
    poll_wait(filep, &rwq, poll_tab);
    poll_wait(filep, &wwq, poll_tab);
    if (user_readable())
